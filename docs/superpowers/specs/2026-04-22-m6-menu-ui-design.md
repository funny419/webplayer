...
| 입력 | 메뉴 닫힘 상태 | 메뉴 열림 상태 |
|------|--------------|--------------|
| `I` 키 | 인벤토리 탭으로 열기 + `scene.pause()` | 메뉴 닫기 (토글) + `scene.resume()` |
| `Q` 키 | 퀘스트 탭으로 열기 + `scene.pause()` | 메뉴 닫기 (토글) + `scene.resume()` |
| `ESC` 키 | — | 메뉴 닫기 + `scene.resume()` |

**키 바인딩 구현 방식:**
- `I`, `Q` 키: Phaser `addKey()` — `scene.pause()` 후에도 Phaser input은 수신되므로 동작함
- `ESC` 키: `document.addEventListener('keydown')` DOM 레벨로 등록 — Phaser pause와 독립적으로 확실히 동작

WorldScene이 pause 상태에서는 `update()` 루프가 멈추므로 전투·이동 입력이 차단된다. 기존 `Z`(근접), `X`(원거리) 키와 충돌 없음.
...
```
InventorySystem.toJSON()       →  renderInventory()   ← 실제 존재하는 메서드
QuestSystem.getAllQuests()      →  renderQuestLog()    ← 신규 추가 필요 (아래 참조)
...
`SaveManager.save()`는 `playtime`, `player`(PlayerStats), `inventory`, `quests`, `puzzles` 전체를 인자로 받는다. `MenuOverlay`는 이 데이터를 직접 조립하지 않으며, **WorldScene이 세이브 데이터를 조립해 콜백으로 전달**한다.

`MenuOverlay` 생성자에 `getSaveData: () => Omit<SaveData, 'version' | 'timestamp'>` 콜백을 추가한다:
...
constructor(
  ...
  getSaveData: () => Omit<SaveData, 'version' | 'timestamp'>
)
```

세이브 버튼 클릭 시: `saveManager.save(getSaveData())` 호출.
...
- 세이브 탭 비활성화 조건: `WorldScene.currentArea === 'scene_haven'` 확인
  - `currentArea: string` 프로퍼티를 `WorldScene`에 신규 추가한다 (현재 미존재)
  - 초기값: `'scene_haven'` (시작 지역), 지역 전환 시 갱신
