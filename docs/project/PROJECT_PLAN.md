...
| 마일스톤 | 내용 |
|----------|------|
| M1 | 하네스 구성, 팀 스폰, GDD 확정 |
| M2 | 플레이어 이동·기본 전투·첫 지역 맵 |
| M3 | 퀘스트·인벤토리·NPC 대화 시스템 |
| M4 | 전체 지역 + 보스 5종 + 저장 시스템 |
| M5 | 메인 퀘스트 완주 + 스토리 엔딩 |
| M6 | UI 폴리싱·버그 픽스·QA |
| M7 | Android WebView 래핑·스토어 배포 |
...
- **완료 기준**: GitHub Issue Close + 프로젝트 보드 Done 동시 처리
- **웹 완료 조건**: `vite build` 성공 확인 후 Done 처리
- **기획 변경**: GDD 확정 후 변경 시 사용자 승인 필요, `docs/GDD.md`에 변경 이력 기록
